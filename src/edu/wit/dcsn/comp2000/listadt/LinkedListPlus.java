 ...
 * @version 4.0
 * 
 * @param <T> the class (or an ancestor class) of objects to be stored in the
 *        list
 */
public class LinkedListPlus<T extends Comparable<? super T>> implements ListInterface<T>, Iterable<T> {
	...
	@Override
	public void sort() {
		this.firstNode = _sort(this.firstNode, false);
	}

	private Node _sort(Node node, Boolean shuffle) {
		...
		Node l = _sort(node, shuffle);

		// Apply mergeSort on right list
		Node r = _sort(temp, shuffle);

		// Merge the left and right lists
		if (shuffle) {
			return shuffleMerge(l, r, new Random());
		} else {
	...
	@Override
	public void shuffle() {
		_sort(this.firstNode, true);
	}

	Node shuffleMerge(Node a, Node b, Random rand) {
		Node result = null;
		/* Base cases */
		if (a == null)
			return b;
		...
		if (rand.nextInt(((1 - -1) + 1) - 1) <= 0) {
			result = a;
			result.next = shuffleMerge(a.next, b, rand);
		} else {
			result = b;
			result.next = shuffleMerge(a, b.next, rand);
		}
		return result;
